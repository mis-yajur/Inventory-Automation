    ...
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl bg-gradient-to-r from-slate-900 via-slate-900/90 to-emerald-950/40 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-400 font-extrabold text-[10px] tracking-wider uppercase border border-emerald-500/30">
              Yajur Fibres & Textiles Ltd
            </span>
            <span className="text-slate-500 text-xs">• Live Warehouse Portal</span>
          </div>
          <h1 className="text-xl font-extrabold text-slate-100 mt-1">
            Inventory & Stock Control Executive Dashboard
          </h1>
          <p className="text-xs text-slate-400 mt-0.5">
            Real-time stock valuation, reorder levels, consumption velocity, and store exceptions
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => onNavigateTab('material-issue')}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl">
          ...
          <div className="text-xl font-black text-slate-300 mt-2">
            {formatCurrency(nonMovingValue)}
          </div>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-slate-800/50 transition">
                      ...
                      <td className="p-3 text-right font-mono text-cyan-400">
                        {item.avgDailyConsumption > 0 ? `${Math.round(item.availableQty / item.avgDailyConsumption)} days` : 'N/A'}
                      </td>
